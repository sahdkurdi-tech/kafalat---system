...
export function loadListData(listId) {
    ...
    let headerHTML = `<tr><th width="30px"></th>
    <th width="40px" class="text-center"><input type="checkbox" id="selectAllCheckbox" onchange="window.toggleSelectAll(this)" style="transform: scale(1.3); cursor: pointer;"></th>
    <th width="50px">#</th>`; 
    ...
    unsubscribeList = onSnapshot(q, (snap) => {
        ...
        snap.forEach(docSnap => {
            ...
            allRowsHTML += `<tr data-id="${docSnap.id}" style="${rowStyle}">
                ...
                <td class="text-center" style="vertical-align: middle; ${rowStyle}"><input type="checkbox" class="row-checkbox" value="${docSnap.id}" style="transform: scale(1.3); cursor: pointer;"></td>
            ...
            visibleFields.forEach((f, index) => {
                ...
                if (index === 0) {
                    const starIcon = d.isStarred ? '<i class="fas fa-star text-warning ms-1" title="تایبەت"></i> ' : '';
            ...
            if (isAdmin) {
                ...
                aidStatusHTML = `
                <button class="btn-act-custom text-success fw-bold" onclick="window.updateAidStatus('${docSnap.id}', 'received')">
                    <i class="fas fa-hand-holding-usd"></i> هاوکاری وەرگرت
                </button>
                <button class="btn-act-custom text-danger fw-bold" onclick="window.updateAidStatus('${docSnap.id}', 'not_received')">
                    <i class="fas fa-times-circle"></i> هاوکاری وەرنەگرت
                </button>
                <button class="btn-act-custom text-secondary fw-bold" onclick="window.updateAidStatus('${docSnap.id}', 'clear')">
                    <i class="fas fa-eraser"></i> لابردنی دۆخ
                </button>
                `;
            }
            ...
            allRowsHTML += `<td class="text-nowrap text-center" style="vertical-align: middle; ${rowStyle}">
                <div class="action-menu">
                    ...
                    <div id="row-${docSnap.id}" class="menu-dropdown">
                        ...
                        ${isAdmin ? `<button class="btn-act-move text-info" onclick="window.moveToList('${docSnap.id}', '${dataStr}')"><i class="fas fa-exchange-alt"></i> گواستنەوە بۆ لیستیتر</button>` : ''}
        ...
        document.querySelectorAll('.row-checkbox').forEach(cb => {
            cb.addEventListener('change', window.updateBulkActionsVisibility);
        });
...
function getDynamicFormHTML(listId, existingData = {}) {
    ...
    html += `
        <div class="form-check form-switch p-0 d-flex align-items-center gap-2 bg-light rounded p-2 border mt-2">
            <input class="form-check-input m-0" type="checkbox" id="inp-is-starred" ${isStarredChecked} style="width: 2.5em; height: 1.5em; cursor: pointer;">
            <label class="form-check-label fw-bold cursor-pointer text-warning" for="inp-is-starred" style="cursor: pointer;">
                <i class="fas fa-star ms-2"></i> نیشانەکردنی بە ئەستێرە (تایبەت)
            </label>
        </div>
    `;
    // ==========================================
...
export function viewDetails(dataStr) {
    ...
    const listColor = listObj ? listObj.color : '#4e73df';
    ...
    if (item.aidStatus === 'received') aidBadge = '<span class="badge bg-success"><i class="fas fa-check"></i> وەرگرتووە</span>';
    else if (item.aidStatus === 'not_received') aidBadge = '<span class="badge bg-danger"><i class="fas fa-times"></i> وەرنەگرتووە</span>';

    let callBadge = '';
    if (item.callStatus === 'answered') callBadge = '<span class="badge bg-success"><i class="fas fa-phone"></i> وەڵامی داوە</span>';
    else if (item.callStatus === 'no_answer') callBadge = '<span class="badge bg-warning text-dark"><i class="fas fa-phone-slash"></i> وەڵامی نەداوە</span>';
    ...
    if (nameField && item.dynamic && item.dynamic[nameField.id]) {
        realName = item.dynamic[nameField.id];
    }
        ...
        <div class="col-12 mb-3 text-center">
            <div class="p-3 rounded-4" style="background: linear-gradient(135deg, ${listColor}15, transparent); border: 1px solid ${listColor}40;">
                ...
                <div class="d-flex justify-content-center flex-wrap gap-2">
                    ...
                    ${callBadge ? `<span class="fs-6 shadow-sm">${callBadge}</span>` : ''}
                </div>
            </div>
        </div>
    `;
    ...
    visibleFields.forEach(f => {
        let val = '-';
        if (f.type === 'sys_name' || f.type === 'sys_amount') return; 
        ...
        if (val !== '-' && val !== '' && val !== null && val !== undefined) {
            if (f.type === 'date' || f.label.includes('بەروار') || f.label.includes('تاکو') || f.label.includes('کۆتایی')) {
                let targetDate = null;
                if (!isNaN(val) && val > 20000 && val < 80000) {
                    const excelBaseDate = new Date(1899, 11, 30); 
                    targetDate = new Date(excelBaseDate.getTime() + val * 24 * 60 * 60 * 1000);
                } else if (typeof val === 'string' && val.includes('-')) {
                    targetDate = new Date(val);
                }
                if (targetDate && !isNaN(targetDate.getTime())) {
                    val = `<span dir="ltr" class="fw-bold text-primary">${targetDate.toISOString().split('T')[0]}</span>`;
                }
            }
                ...
                <div class="col-12 col-sm-6">
                    <div class="card h-100 border border-light shadow-sm" style="background-color: #fcfcfc;">
                        <div class="card-body p-2 d-flex flex-column justify-content-center">
                            <small class="text-muted fw-bold mb-1"><i class="fas fa-circle ms-1" style="font-size: 8px; color:${listColor}"></i> ${f.label}</small>
    ...
    Swal.fire({
        ...
        didOpen: () => {
            ...
            if(popup) popup.style.borderTopColor = listColor;
        }
    });
}
...
export async function updateAidStatus(id, status) {
    try {
        ...
        let msg = 'دۆخەکە سڕایەوە';
        let icon = 'info';
        if (status === 'received') { msg = 'تۆمارکرا: هاوکاری وەرگرتووە'; icon = 'success'; }
        else if (status === 'not_received') { msg = 'تۆمارکرا: هاوکاری وەرنەگرتووە'; icon = 'warning'; }
        
        Swal.fire({ icon: icon, title: msg, timer: 1500, showConfirmButton: false });
    } catch (e) {
        console.error(e);
        Swal.fire('هەڵە', e.message, 'error');
    }
}
...
export async function saveCurrentListToArchive() {
    ...
    const { value: monthLabel } = await Swal.fire({
        ...
        text: 'تەواوی ناوە چالاکەکانی ئەم لیستە دەکرێنە ئەرشیف بەپێی دۆخی (وەرگرت/وەرنەگرت)',
        input: 'text',
    ...
    try {
        ...
        if (snapshot.empty) { 
            Swal.fire({
                icon: 'warning',
                title: 'هیچ ناوێکی چالاک نەدۆزرایەوە!',
                text: 'هیچ کەسێک لەم لیستەدا بوونی نییە بۆ ئەوەی بکرێتە ئەرشیف.'
            }); 
            return; 
        }
        ...
        snapshot.forEach(doc => { 
            ...
            if (data.aidStatus === 'received') receivedCount++;
            else if (data.aidStatus === 'not_received') notReceivedCount++;
        });
        ...
        Swal.fire({ 
            icon: 'success', 
            title: 'بە سەرکەوتوویی ئەرشیف کرا', 
            html: `<b>کۆی گشتی خێزانەکان: ${allData.length}</b><br><br>
                   <span class="text-success">هاوکاری وەرگرت: ${receivedCount}</span><br>
                   <span class="text-danger">هاوکاری وەرنەگرت: ${notReceivedCount}</span>` 
        });
...
window.bulkUpdateAidStatus = async function(status) {
    const selectedCheckboxes = document.querySelectorAll('.row-checkbox:checked');
    const selectedIds = Array.from(selectedCheckboxes).map(cb => cb.value);
    
    if(selectedIds.length === 0) return;

    Swal.fire({title: 'چاوەڕوان بە...', didOpen: () => Swal.showLoading()});
    ...
    try {
        ...
        selectedIds.forEach(id => {
            const ref = doc(db, "lists", window.currentListId, "beneficiaries", id);
            batch.update(ref, { aidStatus: statusValue });
        });
        ...
        Swal.fire('سەرکەوتوو', `دۆخی (${selectedIds.length}) ناو گۆڕدرا بەیەکەوە`, 'success');
    } catch (error) {
        console.error(error);
...
window.updateAidStatus = updateAidStatus;
window.toggleSelectAll = window.toggleSelectAll;
window.updateBulkActionsVisibility = window.updateBulkActionsVisibility;
window.bulkUpdateAidStatus = window.bulkUpdateAidStatus;